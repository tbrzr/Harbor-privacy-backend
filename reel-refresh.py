...
def _hrf_reel_bg(accent):
    """Deterministic per-accent pick between the classic DARK teal field and
    the near-black hero ink (Tim's favorite, matched to the static carousel's
    new "GRADIENT" BG_FIELDS option) -- reels already draw their own accent
    glow on top via radialGradient #hg, so this only swaps the base fill."""
    return ce.HERO_INK if ce.ACCENTS.index(accent) % 2 == 0 else ce.DARK
...
def hrf_scene_svg(accent, kind, hook="", reveal="", steps=None, n_steps=0, cta_text="", url=HRF_URL):
  ...
  <rect width="{W}" height="{H}" fill="{_hrf_reel_bg(accent)}"/><rect width="{W}" height="{H}" fill="url(#hg)"/>
