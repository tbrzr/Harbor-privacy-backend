#!/usr/bin/env python3
import os, json, hmac, hashlib, logging, re, random, string, uuid, requests
...
def delete_adguard_client(client_id):
    try:
        r = requests.post(f"{ADGUARD_URL}/control/clients/delete",
            json={"name": client_id}, auth=(ADGUARD_USER, ADGUARD_PASS), timeout=10)
        if r.status_code == 200:
...
def find_client_by_stripe_id(stripe_customer_id):
    try:
...
def send_welcome_email(email, name, client_id, plan, profile_url=""):
    ...
    if plan == "remote":
        ios_btn = f'<p><a href="{profile_url}" style="display:inline-block;background:#00e5c0;color:#0a0e0f;padding:12px 24px;text-decoration:none;font-family:monospace;font-size:13px;">📲 Download iOS DNS Profile</a></p><p style="font-size:12px;color:#6b8a87;">Tap on iPhone/iPad → Settings → General → VPN & Device Management → Install</p>' if profile_url else "<p style='color:#6b8a87;'>iOS profile will be sent separately.</p>"
...
<ol style="color:#6b8a87;"><li>Settings → Network & Internet → Private DNS</li><li>Private DNS provider hostname</li><li>Enter: <strong style="color:#e8f0ef;">{doh}</strong></li><li>Save</li></ol>
...
<p>Hi {name},</p><p>Thanks for booking. I'll be in touch within 24 hours to schedule your visit.</p>
<p style="border-top:1px solid #1e2a2d;padding-top:24px;color:#6b8a87;">Questions? Reply or text <strong style="color:#e8f0ef;">781-974-6196</strong><br>— Tim<br><a href="https://harborprivacy.com" style="color:#00e5c0;">harborprivacy.com</a></p>
</div>'''
    ...
    def do_POST(self):
        ...
        try:
            ...
            if etype == "checkout.session.completed":
                ...
                customer = find_client_by_stripe_id(stripe_id)
                if customer:
                    cid = customer.get("client_id", "")
                    remove_from_allowed_clients(cid)
                    delete_adguard_client(cid)
                    send_cancellation_email(customer.get("email", ""), customer.get("name", ""))
                    log.info(f"Deactivated: {cid}")
