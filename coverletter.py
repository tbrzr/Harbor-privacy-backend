...
def create_resume_review():
    ...
    email = data.get('email', '').strip()
    tone = data.get('tone', 'professional').strip()
    target_length = data.get('target_length', 'standard').strip()
...
def generate_cover_letter(job_id):
    ...
    tone_instructions = {
        ...
        'friendly': 'Write in a warm, friendly tone. Approachable, enthusiastic, and genuine — like someone they\'d actually enjoy working with.',
        'executive': 'Write in an executive tone. Strategic, decisive, results-first. Lead with business impact and leadership credibility.'
    }.get(tone, 'Write in a polished, professional tone.')
    
    word_targets = {
        'short': 'Keep it under 200 words. Brief and punchy.',
...
def generate_resume_review(job_id):
    ...
    tone = job.get('tone', 'professional')
    target_length = job.get('target_length', 'standard')

    tone_desc = {
        'professional': 'Clean, polished, ATS-optimized language.',
        'executive': 'Executive-level language. Strategic, results-focused, leadership-oriented.',
        'technical': 'Technical precision. Specific tools, frameworks, metrics, and quantified impact.',
        'creative': 'Creative field voice. Shows personality, originality, and real-world impact.',
        'friendly': 'Warm and approachable, yet fully professional.'
    }.get(tone, 'Clean, polished, ATS-optimized language.')

    length_desc = {
        'concise': 'Trim aggressively. Cut every filler phrase. Each line must earn its place.',
        'standard': 'Keep similar overall length. Improve quality without major length changes.',
